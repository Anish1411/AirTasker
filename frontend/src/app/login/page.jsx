...
const RegisterPage = () => {
  ...
  const handleSubmit = async(e) => {
    ...
    try{
      ...
      const req = await fetch('http://localhost:1336/api/auth/local', reqOptions);
      ...
      toast.error("Invalid conditionals or user doesn't exist")
    }
  };
      ...
      <Grid container justifyContent="center" className='Form'>
        <form onSubmit={handleSubmit}>
          <Grid container spacing={1}>
            ...
            <Box style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', marginTop: '25px', marginBottom: '10px' }}>
              <Typography style={{ fontSize: '12px', color: '#74788D', fontWeight: '400', marginLeft: '10px' }} variant='h6'>Must be 6 Characters at Least</Typography>
              <Link style={{ color: '#4c40ed', marginLeft: '100px' }} href='./forgot'>Forgot password?</Link>
            </Box>
        ...
        <Grid item xs={12}>
          <Link className='link' href="/register">
            <Typography className='loginText' variant="body2" sx={{ color: '#74788D', fontSize: 14 }}>
              Don&apos;t have an account? <span style={{ color: '#4c40ed', textDecoration: 'underline', fontWeight: '600', fontSize: 14 }}>Register</span>
            </Typography>
          </Link>
        </Grid>
      </Grid>
